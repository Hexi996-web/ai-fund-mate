...
function ChangeBar({ value }) {
  ...
  const label = Number.isFinite(value) ? `${value >= 0 ? '+' : ''}${value.toFixed(1)}%` : '—'
  return <span className="change-visual" title="可比基金当前规模相较2025年末基准规模的变化率"><i className={value >= 0 ? 'up' : 'down'} style={{width:`${width}%`}} /><b>{label}</b></span>
}
  ...
  return <main className="workspace-main research-pool decision-mode">
    ...
    <section className="decision-layout">
      ...
      {active && <section className="decision-detail"><div className="decision-detail__title"><div><small>当前研究对象</small><h2>{active.name}</h2></div><span>季度序位 {ranked.indexOf(active)+1}</span></div>
        {activeEvidence ? <><h3>四层证据快照</h3><div className="evidence-four"><article><small>结构驱动</small><strong>{activeEvidence.structure.signal}</strong><span>{activeEvidence.structure.source}</span></article><article><small>板块相对估值</small><strong>{activeEvidence.enterprise.value ? `${activeEvidence.enterprise.value}倍 PE · ${activeEvidence.enterprise.level}` : '数据缺失'}</strong><span>候选池中位数 {activeEvidence.enterprise.peerMedian}倍 · 排名 {activeEvidence.enterprise.rank} · 较中位数 {activeEvidence.enterprise.relativeToMedian >= 0 ? '+' : ''}{activeEvidence.enterprise.relativeToMedian}%</span><em>{activeEvidence.enterprise.note}</em></article><article><small>资产承载</small><strong>{activeEvidence.assets.boardName} · {yi(activeEvidence.assets.totalMarketCapYi)}</strong><span>流通市值 {yi(activeEvidence.assets.floatMarketCapYi)} · 流通比 {pct(activeEvidence.assets.floatRatio)}</span><i style={{width:`${activeEvidence.assets.floatRatio || 0}%`}} /></article><article><small>产品供给</small><strong>{active.market.count}只 · {active.market.state}</strong><span>近12月新发 {active.market.launched12.length}只 · 规模变化 {pct(active.market.scaleGrowth)}</span></article></div><p className="evidence-source">公开数据快照 {evidence.updateTime} · 真实数据与代理指标分开展示</p></> : null}
