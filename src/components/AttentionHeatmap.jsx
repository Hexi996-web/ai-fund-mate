...
function CandidateDetail({ item, onSelectCore }) {
  ...
  return <aside className="attention-detail">
    ...
    <p>{item.thesis}</p>
    {item.proof?<p className="attention-proof">数据：{item.proof.attention.source} · {item.proof.validation.source} · {item.proof.validation.asOf}</p>:<p className="attention-proof">待完成公开注意力、基金市场和可投资资产的统一映射，暂不生成坐标。</p>}
...
export function AttentionHeatmap({ onSelectCore }) {
  ...
  return <section className="attention-section" aria-label="未来社会注意力方向热力图">
    ...
    <div className="attention-grid">
      ...
      <div className="attention-pending"><div><strong>待数据映射 · {pending.length}</strong><small>保留在母池，不参与坐标与核心10排序</small></div><section>{pending.map((item)=><button type="button" className={selectedId===item.id?'active':''} onClick={()=>setSelectedId(item.id)} key={item.id}><i/>{item.name}</button>)}</section></div>
