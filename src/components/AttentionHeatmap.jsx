...
function CandidateDetail({ item, onSelectCore }) {
  ...
  return <aside className="attention-detail">
    ...
    <p>{item.thesis}</p>
    <dl><div><dt>媒体注意力代理</dt><dd>{item.attention}</dd></div><div><dt>市场验证代理</dt><dd>{item.industry}</dd></div><div><dt>资产承载</dt><dd>{item.capacity}</dd></div></dl>
...
export function AttentionHeatmap({ onSelectCore }) {
  ...
  const [snapshot,setSnapshot] = useState({items:[],verifiedCount:0,universeCount:ATTENTION_POOL.length,generatedAt:''})
  ...
  const candidates = useMemo(()=>{
    ...
    return ATTENTION_POOL.flatMap((item)=>{
      ...
      return proof?[{...item,attention:proof.attention.score,industry:proof.validation.score,capacity:proof.capacity.score,proof}]:[]
    })
  },[snapshot.items])
  ...
  return <section className="attention-section" aria-label="未来社会注意力方向热力图">
    ...
    <div className="attention-grid">
      <div className="attention-canvas">
        ...
        <div className="quadrant quadrant--lead"><b>提前预研区</b><strong>事实开始领先，社会尚未充分定价</strong></div>
        <div className="quadrant quadrant--resonance"><b>产业—注意力共振区</b><strong>产业兑现与社会关注同步增强</strong></div>
        <div className="quadrant quadrant--seed"><b>结构萌芽区</b><strong>长期逻辑存在，产业证据仍不足</strong></div>
        <div className="quadrant quadrant--risk"><b>叙事验证区</b><strong>公众关注跑在企业兑现之前</strong></div>
        {visible.map((item)=><button type="button" aria-label={`${item.name}，社会注意力${item.attention}，产业兑现${item.industry}`} title={item.name} className={`attention-dot attention-dot--${STAGE_CLASS[item.stage] || 'seed'} ${item.bucket==='核心10'?'is-core':''} ${selectedId===item.id?'is-active':''}`} style={{left:`${item.industry}%`,bottom:`${item.attention}%`,width:`${18+item.capacity*.18}px`,height:`${18+item.capacity*.18}px`}} onClick={()=>setSelectedId(item.id)} key={item.id}>{showLabels || selectedId===item.id ? <span>{item.name}</span> : null}</button>)}
