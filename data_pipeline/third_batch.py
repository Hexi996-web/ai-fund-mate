...
def normalize_fx(df):
    ...
    return [{"indicator_id":"usd_cny_gold","date":_date(r["日期"]),"value":float(r["央行中间价"])/100,"unit":"人民币/美元","dimension":"央行中间价","source_id":"akshare","upstream":"中国银行/中国人民银行"},{"indicator_id":"usd_cny_hk","date":_date(r["日期"]),"value":float(r["央行中间价"])/100,"unit":"人民币/美元","dimension":"央行中间价","source_id":"akshare","upstream":"中国银行/中国人民银行"}]
def normalize_southbound(df):
    rows=df[df["资金方向"]=="南向"]; d=rows["交易日"].max(); rows=rows[rows["交易日"]==d]
    return [{"indicator_id":"southbound_turnover","date":_date(d),"value":float(rows["成交净买额"].sum()),"unit":"亿元","dimension":"南向成交净买额","source_id":"akshare","upstream":"沪深港通公开数据"}]
