...
test('theme workspace shows scores, evidence, related funds, and honest history status', async ({ page }) => {
  ...
  await expect(page.getByText('黄金', { exact: true }).first()).toBeVisible()
  await expect(page.getByText('红利基金', { exact: true }).first()).toBeVisible()
  await expect(page.getByText('债券基金', { exact: true }).first()).toBeVisible()
