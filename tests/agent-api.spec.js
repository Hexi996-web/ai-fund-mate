...
test('normalizes whitelisted tool calls and safe evidence links', async () => {
  ...
  process.env.AGENT_API_KEY = 'test-key'
  process.env.AGENT_MODEL = 'test-model'
  global.fetch = async () => new Response(JSON.stringify({ choices: [{ message: { tool_calls: [{ type: 'function', function: { name: 'switch_workspace', arguments: '{"workspace":"公募基金简报"}' } }] } }] }), { status: 200, headers: { 'Content-Type': 'application/json' } })
  const result = await invoke({ method: 'POST', headers: { 'x-forwarded-for': 'agent-tools' }, body: { messages: [{ role: 'user', content: '打开公募基金简报' }], context: { sources: [{ label: '基金快照', href: '/fund_products.json' }, { label: '危险链接', href: 'https://example.com' }] } } })
  expect(result.status).toBe(200)
  expect(result.body.actions).toEqual([{ name: 'switch_workspace', label: '切换工作板块', arguments: { workspace: '公募基金简报' } }])
