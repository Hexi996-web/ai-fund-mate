...
test('shows a global market brief and a decision-oriented opportunity matrix', async ({ page }) => {
  ...
  await expect(page.getByLabel('板块基金排序')).toHaveValue('dailyDesc')
  const firstDescending = await page.locator('.sector-fund-table > div').nth(1).innerText()
  await page.getByLabel('板块基金排序').selectOption('dailyAsc')
  const firstAscending = await page.locator('.sector-fund-table > div').nth(1).innerText()
